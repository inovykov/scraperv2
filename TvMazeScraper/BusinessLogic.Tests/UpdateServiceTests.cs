...
namespace BusinessLogic.Tests
{
    public class UpdateServiceTests
    {
        ...
        private readonly Mock<IIntegrationSagaFactory> _integrationSagaFactory;
        ...
        {
            _integrationDal.Setup(d => d.GetSagaInProgressAsync(_cancellationToken)).ReturnsAsync(() => _integrationSaga);

            var processingResult = await _updateService.StartUpdateProcessAsync(_cancellationToken);
            
            _integrationSagaFactory.Verify(f => f.CreateIntegrationSaga(It.IsAny<IEnumerable<int>>(),It.IsAny<DateTime>()), Times.Never);
        ...
        {
            _integrationDal.Setup(i => i.GetSagaInProgressAsync(_cancellationToken))
        ...
        public async void Saga_Completed_When_No_More_Items_To_Process()
        {
            ...
            _integrationDal.Verify(i => i.SetSagaStateAsync(It.IsAny<Guid>(), It.Is<SagaStates>(x => x == SagaStates.Completed), _cancellationToken), Times.Once);
        }
        ...
        public async void Verify_Saga_Factory_Receives_Proper_Keys_For_Saga_Creation()
        {
            ...
            _integrationSagaFactory.Verify(s => s.CreateIntegrationSaga(It.Is<IList<int>>(x => Equals(x, listOfOutdatedShowInfos)), It.IsAny<DateTime>()), Times.Once) ;
        }
